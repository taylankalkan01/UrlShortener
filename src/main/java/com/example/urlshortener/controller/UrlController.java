...
public class UrlController {
    ...
    public ResponseEntity<List<ShortUrlDto>> getAllUrls() {
        return ResponseEntity.ok().body(shortUrlDtoConverter.convertToDto(service.getAllShortUrl()));
    }


    @GetMapping("/show/{code}")
    public ResponseEntity<ShortUrlDto> getUrlByCode(@Valid @NotEmpty @PathVariable String code) {
        return ResponseEntity.ok().body(shortUrlDtoConverter.convertToDto(service.getUrlByCode(code)));
    }
